...
// pra Supabase (tabela "ugc_leads") porque no Vercel cada requisição pode
// cair numa instância serverless diferente, sem disco compartilhado — um
// arquivo local gravado numa invocação não é visto pela próxima. Prefixo
// "ugc_" (em vez de só "leads") porque o projeto Supabase é compartilhado
// com outra ferramenta sua que já tinha uma tabela "leads" com schema
// diferente — ver supabase/setup-new-project.sql pra criar a tabela
// (passo manual único, no SQL Editor do Supabase — mesma tabela serve
// local (server.js) e Vercel).
...
// Qualquer campo que chegar num POST /api/leads e NÃO estiver nesta lista
// (ex: tipo_campanha/experiencia_ugc/ideia_campanha do pop-up da página de
// vendas de Gestão de Campanhas) vira automaticamente parte de
// "answers" dentro de history — não precisa de coluna nova na tabela
// "leads" nem migração nenhuma pra um formulário novo com perguntas
// próprias. Ver upsertLead() abaixo e ANSWER_LABELS/ANSWER_VALUE_LABELS
// em banco-de-leads.js pra dar um rótulo bonito a esses campos na tela.
...
async function listLeads() {
  requireSupabase();
  const { data, error } = await supabase.from('ugc_leads').select('*').order('created_at', { ascending: false });
...
async function findExistingLead(incoming) {
  ...
  const { data, error } = await supabase.from('ugc_leads').select('*');
...
async function upsertLead(incoming) {
  ...
  if (existing) {
    ...
    const { data, error } = await supabase.from('ugc_leads').update(patch).eq('id', existing.id).select().single();
    if (error) { console.error('Supabase (upsertLead/update):', error); throw error; }
    return mapLeadRow(data);
  }

  const id = String(Date.now()) + Math.random().toString(36).slice(2, 8);
  const { data, error } = await supabase.from('ugc_leads').insert({
    id: id,
    name: incoming.name || '',
