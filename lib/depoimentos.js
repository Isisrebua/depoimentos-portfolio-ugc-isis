...
async function listDepoimentos(statusFilter) {
  requireSupabase();
  let query = supabase.from('ugc_depoimentos').select('*').order('created_at', { ascending: false });
...
async function updateDepoimentoStatus(id, status) {
  requireSupabase();
  // .select('id') em vez de .select() (linha inteira): o front-end já
  // atualiza a tela de forma otimista antes desta chamada terminar (ver
  // banco-de-leads.js) e não usa o corpo da resposta — só precisamos
  // saber se o id existia (pra devolver 404 corretamente), então pedimos
  // a menor coluna possível de volta, reduzindo o payload da resposta do
  // Postgres/PostgREST nesse round-trip único de UPDATE+SELECT.
  const { data, error } = await supabase
    .from('ugc_depoimentos')
  ...
  return data ? { id: data.id, status: status } : null;
}
